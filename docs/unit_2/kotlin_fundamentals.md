...
Определение делегата c помощью ключевого слова `by`
```kt
var varName by delegateObject
```
`delegateObject` обрабатывает функции установки и получения для свойств.

Создание интерфейса. Интерфейсы служат для достижения абстракции.
```kt
interface name {
    body
}
